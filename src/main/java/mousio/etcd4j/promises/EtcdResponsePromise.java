...
public class EtcdResponsePromise<T> {
    ...
    class MyHandler implements GenericFutureListener<Promise<T>> {
        private final EtcdResponsePromise<T> _outer;

        MyHandler (EtcdResponsePromise<T> outer){
            _outer = outer;
        }
    ...
    private final GenericFutureListener<Promise<T>> promiseHandler = new MyHandler(this);
